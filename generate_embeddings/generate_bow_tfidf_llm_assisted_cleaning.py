"""
Generate TF-IDF Bag-of-Words embeddings from clinical notes.
...
3. Generates TF-IDF weighted BoW vectors using TfidfVectorizer
...
def main():
    """Generate TF-IDF BoW embeddings."""
    print("="*80)
    print("TF-IDF BAG-OF-WORDS EMBEDDING GENERATION")
    print("="*80)
    ...
    # Generate TF-IDF BoW vectors
    print("\nGenerating TF-IDF BoW vectors...")
    ...
    X_tfidf_bow = vectorizer_tfidf.transform(corpus)

    print(f"TF-IDF matrix shape: {X_tfidf_bow.shape}")
    print(f"Number of features: {len(vectorizer_tfidf.vocabulary_)}")

    # Perform SVD
    print("\nPerforming SVD...")
    u_tfidf, s_tfidf, vh_tfidf = np.linalg.svd(X_tfidf_bow.todense())

    # Use top components (up to number of samples)
    n_components = min(len(data), X_tfidf_bow.shape[1])
    embeddings = u_tfidf[:, 0:n_components]
    ...
    output_file = os.path.join(output_dir, "bow_tfidf_llm_cleaned.csv")
    ...
    print("\n" + "="*80)
    print("TF-IDF BOW EMBEDDING GENERATION COMPLETE")
    print("="*80)
